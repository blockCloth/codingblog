      ...
      <el-row class="site">
        <el-col :xs="24" :sm="8" v-for="(item, index) in linksList" :key="item.id">
          <el-card class="card-hover">
            <div :style="{
              backgroundImage: `url(${item.linkImage || 'http://img.mrzym.top/FgTOrGUz5WJwswSLhPsiGL4DOXe3'
                })`,
            }" :class="[
  'site-item',
  'animate__animated',
...
]" @mouseenter="mouseEnterItem('site', index)" @mouseleave="mouseLeaveItem">
              <div class="left">
                <el-avatar :class="[activeType == 'site' && active == index ? 'avatar-hover' : 'avatar']" fit="cover"
                  :size="80" :src="item.linkUrl || returnUrl(item.linkUrl)">
                  <span class="avatar-font">{{ item.linkName }}</span></el-avatar>
              </div>
              <div :class="['right', activeType == 'site' && active == index ? 'right-hover' : '']">
                <div class="w-[100%] flex justify-between items-center">
                  <span :title="item.linkName" class="name">{{ item.linkName }}</span>
                  <div class="op-icon">
                    <el-icon class="right-icon" @click="goToSite(item.linkUrl)">
                      <Promotion />
                    </el-icon>
                  </div>
                </div>

                <span :style="{ height: activeType == 'site' && active == index ? '4.6rem' : '0' }"
                  :title="item.linkDescription" class="desc">
                  {{ item.linkDescription }}</span>
              </div>
            </div>
          </el-card>
        </el-col>
