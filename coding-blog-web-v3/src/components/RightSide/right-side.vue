...
const goToArticleList = (item) => {
  router.push({ path: "articleList", query: { id: item.postTagId, type: "tag", name: item.description } });
};
          ...
          <el-skeleton :loading="loading" animated>
            ...
            <template #default>
              <RightSideItem icon="icon-gonggao" title="公告" color="#f00">
                <div class="notice-text">{{ configDetail.site.announcement }}</div>
              </RightSideItem>
            </template>
          </el-skeleton>
        </el-card>
              ...
              <RightSideItem icon="icon-localoffer" title="标签">
                <div class="notice-text">
                  <span
                    ...
                    >{{
                      index + 1 == tags.length ? tag.description : tag.description + "&nbsp;&nbsp;"
                    }}</span
                  >
                </div>
              </RightSideItem>
              ...
              <RightSideItem icon="icon-zixun" size="1.4rem" title="网站资讯">
                <div class="site-info">
                  <div class="flex_r_between">
                    <span>文章数目：</span>
                    <GsapCount class="value" :value="configDetail.postSize" />
                  </div>
                  <div class="flex_r_between">
                    <span>运行时间：</span>
                    <span>
                  ...
                  <div class="flex_r_between">
                    <span>博客访问次数：</span>
                    <GsapCount
                      v-if="configDetail.blogView - 0 < 1000"
                      class="value"
                      :value="configDetail.blogView"
                    />
                    <span v-else class="value">{{ numberFormate(configDetail.blogView) }}</span>
                  </div>
                </div>
              </RightSideItem>
            </template>
