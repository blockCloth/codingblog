...
const operate = (type, item) => {
  switch (type) {
    case "detail":
      router.push({ path: "/article", query: { id: item.postsId } });
      break;
    case "tag":
        ...
        >
          <el-card class="card-hover">
            <div class="article-box">
              ...
              >
                <div
                  v-image="item.attribute.articleCoverUrl"
                  class="scale flex justify-center items-center"
                  style="width: 100%; height: 100%"
                >
                  <el-image :src="item.attribute.articleCoverUrl" fit="cover" class="image">
                    <template #error>
              ...
              <div class="article-info flex_c_between">
                ...
                <div class="meta">
                  ...
                  <span class="to_pointer" @click="operate('tag', item)">
                    ...
                    <span
                      class="meta-value"
                      v-for="(tagName, index) in item.postTagName"
                      :key="index"
                    >
                      {{ index == item.postTagName.length - 1 ? tagName : tagName + "、" }}
                    </span>
                  </span>
                  <span class="article-meta__separator"></span>
                  <span class="to_pointer">
                    ...
                    <GsapCount
                      class="meta-value"
                      v-if="item.praiseSize - 0 < 1000"
                      :value="numberFormate(item.praiseSize)"
                    />
                    <span v-else class="meta-value">
                      {{ numberFormate(item.praiseSize) }}
                    </span>
                  </span>
                  <span class="article-meta__separator"></span>
                  <span class="to_pointer">
                    ...
                    <GsapCount
                      class="meta-value"
                      v-if="item.viewSize - 0 < 1000"
                      :value="numberFormate(item.viewSize)"
                    />
                    <span v-else class="meta-value">
                      {{ numberFormate(item.viewSize) }}
                    </span>
                  </span>
                </div>
                <Tooltip
