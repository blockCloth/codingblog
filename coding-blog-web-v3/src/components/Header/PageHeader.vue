  ...
  <div class="page-header fadeIn" :style="getBgCover">
    ...
    <div v-if="route.path != '/article'" class="route-font animate__animated animate__fadeIn">
      ...
      <div class="image-list" v-if="route.path == '/photos' && photoAlbumList.length">
        <div :class="['image-box', route.query.id == item.id ? 'current' : '']" v-for="item in photoAlbumList"
          :key="item.id">
    ...
    <div v-else class="article main-article">
      ...
      <Tooltip width="80%" weight="500" size="2.4rem" color="#fff" align="center" :name="article.posts.postTitle" />
      <div class="animate__animated animate__fadeIn !mt-[20px]">
        <span class="to_pointer">
          ...
          <span class="meta-value">{{ article.posts.postDate }}</span>
        </span>
        <span class="to_pointer">
          <i class="iconfont icon-schedule"></i>
          <span class="meta-label">更新于</span>
          <span class="meta-value">{{ article.posts.postModified }}</span>
        </span>
        <span class="meta-separator"></span>
        <span class="to_pointer">
          <i class="iconfont icon-folder"></i>
          <span class="meta-value">{{ article.termTaxonomy.name }}</span>
        </span>
        <span class="meta-separator"></span>
        <span class="to_pointer">
          <i class="iconfont icon-label_fill"></i>
          <span class="meta-value" v-for="(item, index) in article.postTagList" :key="item">{{
            index + 1 == article.postTagList.length ? item.description : item.description + "、"
          }}</span>
        </span>
        <span class="meta-separator"></span>
        <span class="to_pointer">
          ...
          <GsapCount class="meta-value" v-if="article.praiseSize - 0 < 1000" :value="article.praiseSize" />
          <span v-else class="meta-value">
            {{ numberFormate(article.praiseSize) }}
          </span>
        </span>
        <span class="meta-separator"></span>
        <span class="to_pointer">
          ...
          <GsapCount class="meta-value" v-if="article.viewSize - 0 < 1000" :value="article.viewSize" />
          <span v-else class="meta-value">{{ numberFormate(article.viewSize) }}</span>
        </span>
        <span class="meta-separator"></span>
        <span class="to_pointer">
