<template>
  <el-row>
    <el-col :span="24">
      <el-form class="article-edit-form" ref="dataForm" :rules="rules" :model="editDataModel" label-position="right"
        label-width="0" :show-message="false" :status-icon="false">
        <div class="flex-row-ver-center title-row">
          ...
          <div class="btn-area flex-fixed-item">
            <el-button v-if="editMode === 'm'" type="danger" @click="cancelUpdate">
              取消
            </el-button>
        ...
        <el-form-item prop="postContent">
          <mavon-editor v-model="editDataModel.postContent" ref="md"
    ...
    <el-dialog title="发布选项" :visible="pubDialogShow" :show-close="false" width="800px">
      <el-form ref="pubForm" :rules="rules" :model="editDataModel" label-position="right" label-width="100px">
        ...
        <el-form-item label="专栏">
          <el-cascader v-model="editDataModel.termTaxonomyId" :options="termtaxonomyList"></el-cascader>
        </el-form-item>
        <!-- 摘要 -->
...
import { getArticleById, deleteArticle, createArticle, updateArticle, getTagList, mdEditorUploadImage } from '@/api/articles'
import { deleteArticleCover, uploadUrl, importMdUrl } from '@/api/image'
import { getTermTaxonomyPagedList } from '@/api/termTaxonomy'
  ...
  methods: {
    ...
    getSystemTagList() {
      return getTagList().then(res => {
        ...
        this.alertMessageAndCloseWindow('查询标签列表发生异常，请刷新文章列表重试，将关闭编辑窗口')
      })
    },
    //查询栏目列表方法
    getTermTaxonomyList() {
      return getTermTaxonomyPagedList().then(res => {
        this.termtaxonomyList = this.transformData(res)
      })
    },
    //转换栏目列表方法
    transformData(data) {
      return data
        .map(item => {
          const { termTaxonomyId, description, name, parentId, children } = item;
          ...
          if (children && children.length > 0) {
            newItem.children = this.transformData(children);
          }
          return newItem;
        })
    },
    //判断是否为空
    handleBeforeFilter(node, value, data) {
      // 判断当前节点的 children 是否为空
      if (node.children.length === 0) {
        // 如果 children 为空，返回 false，不显示子级选项
        return false;
      }
      // 如果 children 不为空，返回 true，显示子级选项
      return true;
    },
    //取消发布
    cancelPublish() {
    ...
    // 删除文章封面图方法
    deleteImage() {
      deleteArticleCover({ imageName: this.imageName }).then(res => {
    ...
    handleEditorImgAdd(pos, $file) {
      ...
      mdEditorUploadImage(formdata).then(res => {
        ...
        console
        // let imgUrl = process.env.VUE_IMAGE_URL + 

        this.$refs.md.$img2Url(pos, res.imagePath)
        this.$nextTick(() => {
    ...
    loadData() {
      getArticleById({ postId: this.editId }).then(res => {
        ...
        if (!this.editDataModel.tags) {
          ...
          tempArr.forEach(x => {
            this.selectedTagArray.push(parseInt(x.postTagId))
          })
        }
        // 赋值界面栏目对象
        if (!this.editDataModel.termTaxonomyId) {
          this.editDataModel.termTaxonomyId = res.termTaxonomy.termTaxonomyId
        }
