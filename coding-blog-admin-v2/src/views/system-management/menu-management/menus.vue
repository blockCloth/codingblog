  ...
  <div class="app-container">
    ...
    <div class="table-container">
      <el-table ref="multipleTable" height="calc(100% - 10px)" :key="tableAbout.tableKey" :data="tableAbout.tableData"
        border fit highlight-current-row class="normal-table" row-key="menuId" :tree-props="tableAbout.treeProp"
        default-expand-all>
        ...
        <el-table-column label="是否显示" prop="hidden" align="center">
          <template slot-scope="{ row }">
            <el-switch v-model="row.hidden" :active-value="0" :inactive-value="1"
              @change="handleStatusChange($event, row.menuId)">
            </el-switch>
          </template>
        </el-table-column>
      ...
      <el-form ref="editInfoForm" :model="editDataModel" :rules="editDialog.rules" label-position="right"
        ...
        <el-form-item label="上级菜单" prop="parentId">
          <el-select v-model="editDataModel.parentId" filterable placeholder="上级菜单">
            <el-option v-for="item in parentMenuList" :label="item.label" :value="item.value"
              :key="item.value">{{ item.label }}</el-option>
          </el-select>
        </el-form-item>
...
export default {
  ...
  methods: {
      ...
      this.$refs['editInfoForm'].validate((valid) => {
        if (valid) {
          ...
          saveFunc(qs.stringify(this.editDataModel)).then(() => {
            ...
            this.getList()
            // this.getList().then((newMenus) => {
            //   this.$store.commit('setUserMenus', newMenus);
            // })
            //重置编辑数据模型
    ...
    handleStatusChange(status, menuId) {
      const reqData = qs.stringify({ hidden: status, menuId: menuId })
