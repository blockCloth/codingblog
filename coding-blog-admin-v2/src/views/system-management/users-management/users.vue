<template>
  <div class="app-container">
    ...
    <div class="text-right">
      <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="tableAbout.listQuery.page" background :page-size="tableAbout.listQuery.pageSize" layout="total, sizes, prev, pager, next, jumper" :page-sizes="[10,15, 30, 50, 100]" :total="tableAbout.listQuery.total">
      </el-pagination>
    </div>
    <!-- 详情编辑对话框 -->
    <el-dialog :title="editDialog.title[editDialog.status]" width="500px" :visible.sync="editDialog.visible">
      <el-form ref="editInfoForm" :model="editDataModel" :rules="editDialog.rules" label-position="right" label-width="60px">
        <el-form-item label="头像" v-if="editDialog.status === 1">
          ...
          <el-upload class="avatar-uploader" name="image" v-show="!editDataModel.userUrl" :action="uploadUrl" :headers="{Authorization: getToken()}" :on-success="handleUserImageUploadSuccess" :before-upload="beforeUserImageUpload" :file-list="userImageFileList" :show-file-list="false" :on-remove="handleUserImageRemove">
...
export default {
  ...
  methods: {
    ...
    handleUserImageUploadSuccess(res, file) {
      ...
      this.editDataModel.userUrl = res.result.imagePath
      this.userImageOptionsShow = false
    },

    // 启用/禁用用户事件处理方法
    handleStatusChange(status, userId) {
      const reqData = { status, userId }
    ...
    // 修改按钮点击方法
    handleUpdate(row) {
      getUserInfoById({ userId: row.usersId }).then(res => {
    ...
    handleDelete(row) {
      ...
      this.$confirm(confirmMes, '系统提示', {
        ...
        deleteUser({ userId: row.usersId }).then(() => {
          this.$notify({
            title: '成功',
            message: '删除成功',
            type: 'success',
