  ...
  <div class="app-container">
    <div class="flex-row-ver-center">
      <div class="flex-auto-item">
        <el-input v-model="tableAbout.listQuery.roleName" placeholder="请输入角色名称" clearable></el-input>
      </div>
...
export default {
  ...
  methods: {
    ...
    // 启用/禁用用户事件处理方法
    handleStatusChange(status, roleId) {
      const reqData = qs.stringify({ status, roleId })
    ...
    // 文章搜索方法
    search() {
      this.tableAbout.listQuery.pageNum = 1
      this.getList()
    },
    ...
    handleDelete(row) {
      ...
      this.$confirm(confirmMes, '系统提示', {
        ...
        // const reqData
        deleteRoleBatch({ roleId: row.roleId }).then(() => {
          this.$notify({
            title: '成功',
            message: '删除成功',
            type: 'success',
